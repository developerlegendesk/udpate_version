            ...
            @foreach ($invoices as $item)
                <tr>
                    ...
                    <td style="width: 15%; border-right: 1pt solid black;">
                        ...
                        <?php
                                    $tran = App\Models\Transaction::where('ref',$item->ref)->first();
                                    $entries = App\Models\Entry::where('transaction_id',$tran->id)->get();
                                    $acc = '';
                    ...
                    <?php
                            if(App\Models\InvoiceEntry::where([['invoice_id',$item->id],['description','Total']])->first()){
                                $inv = App\Models\InvoiceEntry::where([['invoice_id',$item->id],['description','Total']])->first();
